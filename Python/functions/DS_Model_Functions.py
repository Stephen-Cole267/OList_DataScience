...
def param_tuning_xgboost(LOGGER, params_grid, dtrain, num_boost_round, nfold, stratified, metrics, early_stopping_rounds, seed):
    ...
    LOGGER.info("XGBoost Parameter Tuning started at {}.".format( time.asctime( time.localtime(start_time) ) ))
    best_params = {}
    max_roc_auc = 0
...
def param_tuning_lgb(LOGGER, params_grid, X_train, y_train, nfold, stratified, metrics):
    ...
    start_time = time.time()
    print('start_time', time.asctime( time.localtime(start_time) ))
    LOGGER.info("LightGBM Parameter Tuning started at {}.".format( time.asctime( time.localtime(start_time) ) ))
    best_params = {}
    max_roc_auc = 0
...
def xgb_hpt_tune(LOGGER, base_params, grid, dtrain, num_boost_round, nfold, stratified, metrics, early_stopping_rounds, seed):
    ...
    print('XGBoost HPT end_time', time.asctime( time.localtime(end_time) ))
    
    LOGGER.info("xgb_hpt_tune finished at {}.".format( time.asctime( time.localtime(end_time) ) ))
...
def lgb_hpt_tune(LOGGER, base_params, grid, X_train, y_train, nfold, stratified, metrics):
    ...
    start_time = time.time()
    print('LightGBM HPT start_time', time.asctime( time.localtime(start_time) ))
    LOGGER.info("lgb_hpt_tune started at {} seconds.".format( time.asctime( time.localtime(start_time) ) ))
    ...
    print('LightGBM HPT end_time', time.asctime( time.localtime(end_time) ))
    
    LOGGER.info("lgb_hpt_tune finished at {}.".format( time.asctime( time.localtime(end_time) ) ))
